...
async function findReading() {
    ...
    try {
      ...
      if (allText.includes('Reading')) {
        ...
        for (const img of images) {
          ...
          if (parent && parent.textContent && parent.textContent.includes('Reading')) {
            let url = img.src.replace('/medium', '').replace('/tiny', '');
  ...
  // Try API search
  try {
    const response = await fetch('https://www.thesportsdb.com/api/v1/json/3/search_all_teams.php?n=Reading');
    const data = await response.json();
    if (data.results && data.results.length > 0) {
      const reading = data.results.find(t => t.strTeam === 'Reading FC' || t.strTeam === 'Reading');
      if (reading && reading.strTeamBadge) {
        const url = reading.strTeamBadge.replace('www.thesportsdb.com', 'r2.thesportsdb.com').replace('/medium', '');
